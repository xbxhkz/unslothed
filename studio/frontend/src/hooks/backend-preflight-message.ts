...
export function preflightStaleMessage(
  ...
  if (kind === WORKING_DIRECTORY_UNAVAILABLE) {
    const cause =
      typeof navigator !== "undefined" && /Win/i.test(navigator.platform ?? "")
        ...
        : "";
    return `Unslothed cannot reach your user folder, so it has nowhere to run from.${cause} Reconnect and try again.`;
  }
  ...
  if (kind === PATH_SETTING_UNRESOLVABLE) {
    const which = setting ? `${setting} points` : "One of Unslothed's folder settings points";
    return `${which} somewhere that cannot be resolved, so Unslothed has nowhere safe to run from. Set it to a full path, such as D:\\unsloth-cache, and try again.`;
  }
  if (disposition === "owned_stale") {
    return "Desktop-owned Unslothed backend is too old for this desktop app. Run `unsloth studio update`, then restart Unslothed.";
  }
  return "Managed Unslothed install is too old. Run `unsloth studio update`.";
}
